...
class ClaudeCodeRemoteCLI {
    constructor() {
        ...
        this.tmuxMonitor = null;
        this.tmuxInjector = null;
        ...
        try {
            ...
            switch (command) {
                ...
                case 'diagnose':
                    ...
                    break;
                case 'interact': // New command for interactive prompt handling
                    await this.handleInteract(args.slice(1));
                    break;
                case '--help':
                case '-h':
                case undefined:
    ...
    async captureCurrentConversation() {
        try {
            ...
            try {
                ...
                currentTarget = execSync('tmux display-message -p "#S:#I.#P"', { 
                    encoding: 'utf8',
                    stdio: ['ignore', 'pipe', 'ignore']
                }).trim();
            } catch (e) {
            ...
            const captureTarget = currentTarget || currentSession;
            const conversation = tmuxMonitor.getRecentConversation(captureTarget);
            const fullTrace = tmuxMonitor.getFullExecutionTrace(captureTarget);
            
            return {
                userQuestion: conversation.userQuestion,
                claudeResponse: conversation.claudeResponse,
                tmuxSession: currentSession,
                tmuxTarget: currentTarget || null,
    ...
    async handleInteract(args) {
        let currentSession = null;
        try {
            currentSession = require('child_process').execSync('tmux display-message -p "#S"', { 
                encoding: 'utf8',
                stdio: ['ignore', 'pipe', 'ignore']
            }).trim();
            if (!currentSession) {
                this.logger.error('Not running in a tmux session. Please run `claude-remote interact` inside a tmux session.');
                process.exit(1);
            }
        } catch (e) {
            this.logger.error('Could not determine tmux session. Are you in tmux? Error:', e.message);
            process.exit(1);
        }
        
        this.logger.info(`Entering interactive mode for tmux session: ${currentSession}`);

        this.tmuxMonitor = new TmuxMonitor(currentSession);
        this.tmuxInjector = new TmuxInjector(this.logger, currentSession); // Pass logger to injector
        ...
        // Set up listener for multi-choice prompts
        this.tmuxMonitor.on('multiChoicePrompt', async (promptData) => {
            await this._handleMultiChoicePrompt(promptData);
        });

        // Set up listener for waitingForInput prompts
        this.tmuxMonitor.on('waitingForInput', async (data) => {
            this.logger.info('Claude is waiting for input. Auto-responding if possible or waiting for multi-choice.');
            // This is primarily to prevent the monitor from getting stuck if it detects
            // a waitingForInput that is not a multi-choice prompt.
            // The handleConfirmations in TmuxInjector should ideally handle simple cases.
        });

        // Start monitoring
        this.tmuxMonitor.start();

        // Keep the process alive. It will exit when tmuxMonitor is stopped
        // or a specific exit command is given.
        process.stdin.resume();

        process.on('SIGINT', async () => {
            this.logger.info('\nExiting interactive mode.');
            this.tmuxMonitor.stop();
            if (this.rl) {
                this.rl.close();
            }
            process.exit(0);
        ...
        // Placeholder for future interactive prompt handling logic
    }

    async _handleMultiChoicePrompt(promptData) {
        // Clear previous output to make the prompt stand out
        console.clear(); 

        const separator = '─'.repeat(process.stdout.columns || 80); // Dynamic separator width

        this.logger.log('\n' + separator);
        this.logger.log('✨ Claude Interactive Prompt ✨');
        this.logger.log(separator);
        this.logger.log(`Question: ${promptData.question}\n`);

        promptData.options.forEach(option => {
            this.logger.log(`  [${option.number}] ${option.text}`);
        });
        ...
        try {
            ...
            while (selectedOption === null) {
                const choice = await question('Type something: ');
                const trimmedChoice = choice.trim();

                // 1. Try to match a numerical option
                const choiceNum = parseInt(trimmedChoice, 10);
                const isPurelyNumeric = !isNaN(choiceNum) && trimmedChoice === choiceNum.toString();
                const matchedOption = promptData.options.find(opt => opt.number === choiceNum);
                
                if (isPurelyNumeric && matchedOption) {
                    selectedOption = choiceNum.toString();
                } 
                // 2. Try to match "N. Custom Text" format, or just a custom text
                else if (trimmedChoice.includes('.') || !isPurelyNumeric) {
                    const parts = trimmedChoice.split('.');
                    const potentialNumberPart = parts[0].trim();
                    const parsedNumber = parseInt(potentialNumberPart, 10);

                    // If it starts with a number and a dot, treat as "N. Custom Text"
                    if (!isNaN(parsedNumber) && potentialNumberPart.length > 0 && trimmedChoice.includes('.')) {
                        const customTextPart = parts.slice(1).join('.').trim();
                        if (customTextPart.length > 0) {
                            selectedOption = customTextPart; // Valid custom text input
                        } else {
                            this.logger.warn('❌ Invalid Input: Custom text cannot be empty after the number and dot.');
                        }
                    } else if (trimmedChoice.length > 0) { // Otherwise, treat as direct custom text input
                         selectedOption = trimmedChoice;
                    } else {
                        this.logger.warn('❌ Invalid Input: Please enter a valid option number or custom text.');
                    }
                }
                // 3. Invalid input that doesn't fit either pattern
                else {
                    this.logger.warn('❌ Invalid Input: Please enter a valid option number or custom text.');
                }
            }

            this.logger.info(`Sending selection to tmux: "${selectedOption}"`);
            const result = await this.tmuxInjector.injectCommand(selectedOption);
            if (result.success) {
                this.logger.info('✅ Selection sent successfully!');
            } else {
                this.logger.error('❌ Failed to send selection:', result.error);
            }
        } catch (error) {
            this.logger.error('❌ Error handling multi-choice prompt:', error.message);
        }
    }
...
Commands:
  ...
  interact                Enter interactive mode for Claude prompts
