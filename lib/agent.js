 ...
 * agent.js
 *
 * npm-packaged version of clw-agent.js.
 * All paths are resolved via config.js (CLWCLW_HOME, CLWCLW_VENDOR_DIR) with
 * fallbacks so both `npm install -g` and repo-checkout workflows work.
 *
...
const { resolveHome, ensureHome, loadDotEnvIfPresent, resolveVendorDir } = require('./config');

function usage() {
  const cmd = process.env.CLWCLW_CLI ? 'clwclw' : 'node agent/clw-agent.js';
  console.log(`Usage:
  ${cmd} heartbeat
  ${cmd} hook <completed|waiting>
  ${cmd} run
  ${cmd} work --channel <name> --tmux-target <target>
...
Env:
  ...
  AGENT_ID                 optional (persisted to data dir)
...
function agentDataDir() {
  ...
  // npm-packaged: use CLWCLW_HOME/data
  return path.join(resolveHome(), 'data');
}
...
function stateInstancesRoot() {
  return path.join(resolveHome(), 'data', 'instances');
}
...
function runLegacyHook(type) {
  // Try vendor dir first (npm install -g), then repo-relative fallback
  const vendorDir = resolveVendorDir();
  let legacyScript = path.join(vendorDir, 'claude-hook-notify.js');

  if (!fs.existsSync(legacyScript)) {
    // Fallback: repo checkout layout (agent/ is sibling of Claude-Code-Remote/)
    const repoFallback = path.join(__dirname, '..', 'Claude-Code-Remote', 'claude-hook-notify.js');
    if (fs.existsSync(repoFallback)) {
      legacyScript = repoFallback;
    } else {
      console.error(`[agent] legacy hook script not found: ${legacyScript}`);
      return 1;
    }
  }

  // Set DOTENV_PATH so vendored script uses the right .env
  const dotenvPath = path.join(resolveHome(), '.env');
  const env = { ...process.env };
  if (fs.existsSync(dotenvPath) && !env.DOTENV_PATH) {
...
async function main(argv) {
  // Load .env: CLWCLW_HOME/.env first, then repo-relative fallback
  const homeEnv = path.join(resolveHome(), '.env');
  loadDotEnvIfPresent(homeEnv);
  ...
  if (cmd === 'run') {
    const vendorDir = resolveVendorDir();
    let legacyDir = vendorDir;
    let legacyEntrypoint = path.join(vendorDir, 'start-all-webhooks.js');

    // Fallback: repo checkout layout
    if (!fs.existsSync(legacyEntrypoint)) {
      const repoFallback = path.join(repoRoot, 'Claude-Code-Remote');
      const repoEntry = path.join(repoFallback, 'start-all-webhooks.js');
      if (fs.existsSync(repoEntry)) {
        legacyDir = repoFallback;
        legacyEntrypoint = repoEntry;
      }
    }
    ...
    // Set DOTENV_PATH for vendored scripts
    const dotenvPath = path.join(resolveHome(), '.env');
    const env = { ...process.env };
    if (fs.existsSync(dotenvPath) && !env.DOTENV_PATH) {
      env.DOTENV_PATH = dotenvPath;
...
// Allow both direct execution and require() from CLI dispatcher
