...
class CommandRelayService extends EventEmitter {
    ...
    _queueCommand(commandData) {
        ...
        void this.coordinator.event('email.command.queued', {
            commandId: queueItem.id,
            sessionId: queueItem.sessionId,
            command: queueItem.command,
            from: queueItem.email?.from,
            subject: queueItem.email?.subject,
            queuedAt: queueItem.queuedAt
        }, `email.command.queued:${queueItem.id}`);
    }
    ...
    async _executeCommand(commandItem) {
        ...
        try {
            ...
            if (success) {
                ...
                void this.coordinator.event('email.command.executed', {
                    commandId: commandItem.id,
                    sessionId: commandItem.sessionId,
                    command: commandItem.command,
                    executedAt: commandItem.executedAt,
                    completedAt: commandItem.completedAt
                }, `email.command.executed:${commandItem.id}`);
            } else {
            ...
            void this.coordinator.event('email.command.failed', {
                commandId: commandItem.id,
                sessionId: commandItem.sessionId,
                command: commandItem.command,
                error: error.message,
                failedAt: commandItem.failedAt,
                retries: commandItem.retries
            }, `email.command.failed:${commandItem.id}`);
