...
class EmailChannel extends NotificationChannel {
    ...
    _getCurrentTmuxTarget() {
        try {
            // Current tmux target: session:window.pane
            const tmuxTarget = execSync('tmux display-message -p "#S:#I.#P"', { 
                encoding: 'utf8',
                stdio: ['ignore', 'pipe', 'ignore']
            }).trim();
    ...
    async _sendImpl(notification) {
        ...
        // Get current tmux target/session and conversation content
        const tmuxTarget = this._getCurrentTmuxTarget();
        const tmuxSession = this._getCurrentTmuxSession();
        if (tmuxTarget || tmuxSession) {
            if (!notification.metadata) notification.metadata = {};
            if (tmuxSession && !notification.metadata.tmuxSession) {
                notification.metadata.tmuxSession = tmuxSession;
            }
            if (tmuxTarget && !notification.metadata.tmuxTarget) {
                notification.metadata.tmuxTarget = tmuxTarget;
            }

            const captureTarget = tmuxTarget || tmuxSession;
            if (captureTarget) {
                const conversation = this.tmuxMonitor.getRecentConversation(captureTarget);
                const fullTrace = this.tmuxMonitor.getFullExecutionTrace(captureTarget);
                if (!notification.metadata.userQuestion) {
                    notification.metadata.userQuestion = conversation.userQuestion || notification.message;
                }
                if (!notification.metadata.claudeResponse) {
                    notification.metadata.claudeResponse = conversation.claudeResponse || notification.message;
                }
                if (!notification.metadata.fullExecutionTrace) {
                    notification.metadata.fullExecutionTrace = fullTrace;
                }
            }
        }
    ...
    async _createSession(sessionId, notification, token) {
        const session = {
            ...
            cwd: process.cwd(),
            tmuxSession: notification.metadata?.tmuxSession || null,
            tmuxTarget: notification.metadata?.tmuxTarget || null,
            notification: {
        ...
        // Use passed tmux info or detect current target/session
        let tmuxSession = notification.metadata?.tmuxSession || this._getCurrentTmuxSession() || 'claude-code-remote';
        let tmuxTarget = notification.metadata?.tmuxTarget || this._getCurrentTmuxTarget();
