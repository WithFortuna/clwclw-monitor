...
const hookScriptPath = hookMode === 'agent' ? agentHookScriptPath : legacyHookScriptPath;
...
function writeEnvFile(values, existingEnv) {
    const orderedKeys = [
        ...
        // Optional coordinator/agent bridge settings (preserve if present)
        'COORDINATOR_URL', 'COORDINATOR_AUTH_TOKEN', 'AGENT_ID', 'AGENT_NAME'
    ];
...
function makeHookCommand(event) {
    ...
    const args = hookMode === 'agent' ? `hook ${event}` : event;
    return `node ${script} ${args}`.trim();
}

function makeLegacyHookCommand(event) {
    const script = legacyHookScriptPath.includes(' ') ? `"${legacyHookScriptPath}"` : legacyHookScriptPath;
    return `node ${script} ${event}`.trim();
}

function makeAgentHookCommand(event) {
    const script = agentHookScriptPath.includes(' ') ? `"${agentHookScriptPath}"` : agentHookScriptPath;
    return `node ${script} hook ${event}`.trim();
}
...
function ensureHooksFile() {
    ...
    if (fs.existsSync(settingsPath)) {
        ...
        try {
            ...
            console.warn(warning(`Existing .claude/settings.local.json is invalid JSON, backed up to ${backupPath}`));
            settings = {};
        }
    }
    ...
    const legacyCompletedCommand = makeLegacyHookCommand('completed');
    const legacyWaitingCommand = makeLegacyHookCommand('waiting');
    const agentCompletedCommand = makeAgentHookCommand('completed');
    const agentWaitingCommand = makeAgentHookCommand('waiting');

    function reconcileHooks(list, desiredCommand, commandsToRemove) {
        const cleaned = [];
        ...
        for (const entry of list) {
            ...
            const hooks = entry.hooks.filter(h =>
                !(h && typeof h === 'object' && typeof h.command === 'string' && commandsToRemove.includes(h.command))
            );
        ...
        const exists = cleaned.some(entry =>
            entry && typeof entry === 'object' && Array.isArray(entry.hooks) && entry.hooks.some(h => h && h.command === desiredCommand)
        );
        ...
        if (!exists) {
            ...
            cleaned.push({
                matcher: '*',
                hooks: [{ type: 'command', command: desiredCommand, timeout: timeout }]
            });
        }
    ...
    const removeStop = hookMode === 'agent' ? [legacyCompletedCommand] : [agentCompletedCommand];
    const removeSubagent = hookMode === 'agent' ? [legacyWaitingCommand] : [agentWaitingCommand];

    settings.hooks.Stop = reconcileHooks(stopHooks, completedCommand, removeStop);
    settings.hooks.SubagentStop = reconcileHooks(subagentHooks, waitingCommand, removeSubagent);
