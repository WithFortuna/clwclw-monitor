...
# Prefer clw-agent wrapper (if present) to preserve legacy notifications AND upload to Coordinator
AGENT_WRAPPER_PATH="$PROJECT_DIR/../agent/clw-agent.js"
if [ -f "$AGENT_WRAPPER_PATH" ]; then
    HOOK_COMPLETED_CMD="node $AGENT_WRAPPER_PATH hook completed"
    HOOK_WAITING_CMD="node $AGENT_WRAPPER_PATH hook waiting"
    echo "🧩 Using agent wrapper hooks: $AGENT_WRAPPER_PATH"
else
    HOOK_COMPLETED_CMD="node $PROJECT_DIR/claude-hook-notify.js completed"
    HOOK_WAITING_CMD="node $PROJECT_DIR/claude-hook-notify.js waiting"
    echo "🧩 Using legacy hooks: $PROJECT_DIR/claude-hook-notify.js"
fi
