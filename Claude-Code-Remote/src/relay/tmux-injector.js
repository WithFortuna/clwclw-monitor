...
class TmuxInjector {
    constructor(logger, sessionName = null, options = {}) {
        this.log = logger || console;
        this.sessionName = sessionName || 'claude-taskping';
        this.target = options && typeof options === 'object' ? (options.target || null) : null;
        this.logFile = path.join(__dirname, '../logs/tmux-injection.log');
        this.ensureLogDir();
    }

    tmuxKeyTarget() {
        return this.target || this.sessionName;
    }
    ...
    async injectCommand(command) {
        return new Promise(async (resolve) => {
            try {
                const target = this.tmuxKeyTarget();
                // 1. Clear input field
                const clearCommand = `tmux send-keys -t ${target} C-u`;
                
                // 2. Send command
                const escapedCommand = command.replace(/'/g, "'\"'\"'");
                const sendCommand = `tmux send-keys -t ${target} '${escapedCommand}'`;
                
                // 3. Send enter
                const enterCommand = `tmux send-keys -t ${target} C-m`;
        ...
        while (attempts < maxAttempts) {
            ...
            if (output.includes('Do you want to proceed?') && 
                ...
                await new Promise((resolve) => {
                    exec(`tmux send-keys -t ${this.tmuxKeyTarget()} '2'`, (error) => {
                        if (error) {
                            ...
                            // Send Enter key
                            setTimeout(() => {
                                exec(`tmux send-keys -t ${this.tmuxKeyTarget()} 'Enter'`, (enterError) => {
                                    if (enterError) {
            ...
            if (output.includes('❯ 1. Yes') || output.includes('▷ 1. Yes')) {
                ...
                await new Promise((resolve) => {
                    exec(`tmux send-keys -t ${this.tmuxKeyTarget()} '1'`, (error) => {
                        if (error) {
            ...
            if (output.includes('(y/n)') || output.includes('[Y/n]') || output.includes('[y/N]')) {
                ...
                await new Promise((resolve) => {
                    exec(`tmux send-keys -t ${this.tmuxKeyTarget()} 'y'`, (error) => {
                        if (error) {
    ...
    async getCaptureOutput() {
        return new Promise((resolve) => {
            const command = `tmux capture-pane -t ${this.tmuxKeyTarget()} -p`;
