...
class TmuxMonitor extends EventEmitter {
    constructor(sessionName = null) {
        ...
        // Patterns to detect a general multiple-choice prompt
        this.multiChoicePatterns = [
            /select an option:/i,
            /choose an action:/i,
            /what would you like to do\?/i,
            /Please choose from the following/i,
            /enter your choice:/i,
            /^\s*(\d+\.\s*[\w\s]+(\r?\n|$))+\s*(?:\r?\n|Select an option:|Enter your choice:)/m // Generic numbered list
        ];
    ...
    _safeCaptureKey(label) {
        const raw = String(label || '').trim();
        if (!raw) return 'default_000000000000';
        const safe = raw
            .replace(/[^a-zA-Z0-9_-]+/g, '_')
            .replace(/^_+|_+$/g, '')
            .slice(0, 48) || 'tmux';
        try {
            const hash = require('crypto').createHash('sha1').update(raw).digest('hex').slice(0, 12);
            return `${safe}_${hash}`;
        } catch (error) {
            return `${safe}_000000000000`;
        }
    }

    _captureFilePath(sessionOrTarget) {
        return path.join(this.captureDir, `${this._safeCaptureKey(sessionOrTarget)}.log`);
    }

    _legacyCaptureFilePath(sessionOrTarget) {
        return path.join(this.captureDir, `${sessionOrTarget}.log`);
    }
    ...
    _analyzeNewContent(newLines) {
        ...
        if (hasTaskCompletion || hasResponseEnd) {
            ...
            // Try to detect and extract a multiple-choice prompt from the recent output
            const multiChoicePrompt = this._detectAndExtractMultiChoicePrompt(recentText);
            if (multiChoicePrompt) {
                console.log('❓ Multiple-choice prompt detected!');
                this.emit('multiChoicePrompt', {
                    type: 'multiChoice',
                    sessionName: this.sessionName,
                    timestamp: new Date().toISOString(),
                    question: multiChoicePrompt.question,
                    options: multiChoicePrompt.options,
                    fullText: recentText
                });
                return; // Stop further analysis if a multi-choice prompt is found
            }
            // Don't constantly trigger waiting notifications for static content
            if (this._shouldTriggerWaitingNotification(recentText)) {
    ...
    /**
     * Detects and extracts a multiple-choice prompt from the given content.
     * @param {string} content The content to analyze (e.g., recent output).
     * @returns {Object|null} An object { question: string, options: Array<string> } or null if no prompt is found.
     */
    _detectAndExtractMultiChoicePrompt(content) {
        const lines = content.split('\n');
        let question = '';
        const options = [];
        let inPrompt = false;
        let potentialQuestionLines = [];

        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();

            // Check for general prompt patterns first
            const isPromptStart = this.multiChoicePatterns.some(pattern => {
                // If it's a generic numbered list, handle it specifically
                if (pattern.source.includes('Generic numbered list')) {
                    return false; // Handled below
                }
                return pattern.test(line);
            });

            if (isPromptStart || inPrompt) {
                if (!inPrompt && isPromptStart) {
                ...
                if (optionMatch) {
                    ...
                    // If we found an option, the question must be above it
                    if (potentialQuestionLines.length > 0 && question === '') {
                        question = potentialQuestionLines.filter(l => !l.match(/^(\d+)\.\s*(.+)/)).join(' ').trim();
                        potentialQuestionLines = []; // Clear after extracting question
                    }
                } else if (inPrompt && line.length > 0 && !line.match(/^[a-zA-Z0-9_ -]+:\s*$/) && !line.match(/^[╭╰│─]+$/)) {
                    // Collect lines that are not options or box characters as potential question text
                    potentialQuestionLines.push(line);
                }
            }

            // A blank line or a prompt for input (like '>') usually signifies the end of the prompt block
            if (inPrompt && (line === '' || line.match(/^\s*>\s*$/) || line.includes('Enter your choice:'))) {
                // If we have options, we're probably done
                if (options.length > 0) {
                    if (question === '') { // Fallback if question wasn't clearly identified before options
                        const filteredQuestionLines = potentialQuestionLines.filter(l => 
                            !this.multiChoicePatterns.some(p => p.test(l)) && !l.match(/^(\d+)\.\s*(.+)/)
                        );
                        question = filteredQuestionLines.join(' ').trim();
                    }
                    if (question === '') { // Use a default if still empty
                        question = "Please make a selection:";
                    }
                    return { question: question, options: options };
                }
                // If no options, reset and continue searching for a better prompt
                inPrompt = false;
        ...
        // Final check if prompt extends to the end of content
        if (inPrompt && options.length > 0) {
            if (question === '') {
                const filteredQuestionLines = potentialQuestionLines.filter(l => 
                    !this.multiChoicePatterns.some(p => p.test(l)) && !l.match(/^(\d+)\.\s*(.+)/)
                );
    ...
    startCapture(sessionName) {
        try {
            const captureFile = this._captureFilePath(sessionName);
    ...
    getRecentConversation(sessionName, lines = 200) {
        try {
            const captureFile = this._captureFilePath(sessionName);
            const legacyCaptureFile = this._legacyCaptureFilePath(sessionName);
            
            const fileToRead = fs.existsSync(captureFile) ? captureFile : (fs.existsSync(legacyCaptureFile) ? legacyCaptureFile : '');
            if (!fileToRead) {
