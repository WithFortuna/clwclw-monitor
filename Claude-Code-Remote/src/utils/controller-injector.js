...
class ControllerInjector {
    ...
    _tmuxSessionFromTarget(targetOrSession) {
        const raw = String(targetOrSession || '').trim();
        if (!raw) return '';
    ...
    _injectTmux(command, targetOrSession) {
        try {
            const target = String(targetOrSession || '').trim();
            if (!target) {
                throw new Error('Tmux target not specified');
            }

            // Check if tmux session exists (target may include window/pane)
            const sessionToCheck = this._tmuxSessionFromTarget(target);
            if (sessionToCheck) {
                try {
                    execSync(`tmux has-session -t ${sessionToCheck}`, { stdio: 'ignore' });
                } catch (error) {
                    throw new Error(`Tmux session '${sessionToCheck}' not found`);
                }
            }
            ...
            // Send command first
            execSync(`tmux send-keys -t ${target} '${escapedCommand}'`);
            // Then send Enter as separate command
            execSync(`tmux send-keys -t ${target} Enter`);
            
            this.logger.info(`Command injected to tmux target '${target}'`);
            return true;
        } catch (error) {
