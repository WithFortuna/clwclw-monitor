...
const CoordinatorClient = require('../../core/coordinator-client');
...
class TelegramWebhookHandler {
    ...
    async _handleMessage(message) {
        ...
        if (!commandMatch) {
            ...
            if (directMatch) {
                await this._processCommand(chatId, directMatch[1], directMatch[2], { userId });
            } else {
        ...
        await this._processCommand(chatId, token, command, { userId });
    }
    ...
    async _processCommand(chatId, token, command, context = {}) {
        ...
        try {
            ...
            const tmuxTarget = session.tmuxTarget || null;
            const injectionTarget = tmuxTarget || tmuxSession;
            await this.injector.injectCommand(command, injectionTarget);
            
            // Send confirmation
            await this._sendMessage(chatId, 
                `✅ *Command sent successfully*\n\n📝 *Command:* ${command}\n🖥️ *Target:* ${injectionTarget}\n\nClaude is now processing your request...`,
                { parse_mode: 'Markdown' });
            
            // Log command execution
            this.logger.info(`Command injected - User: ${chatId}, Token: ${token}, Command: ${command}`);

            // Optional coordinator upload (best-effort, non-blocking)
            void this.coordinator.heartbeat('running', '', { tmux_session: tmuxSession, tmux_target: tmuxTarget || null });
            void this.coordinator.event('telegram.command.sent', {
                token,
                command,
                tmuxSession,
            ...
            void this.coordinator.event('telegram.command.failed', {
                token,
                command,
                chatId,
                userId: context.userId,
                error: error.message,
                sessionId: session.id,
                tmuxSession: session.tmuxSession || 'default',
