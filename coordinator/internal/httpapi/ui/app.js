...
function renderAgents(agents) {
  ...
  if (!agents.length) {
    els.agentsTbody.innerHTML = `<tr><td colspan="6" class="muted">No agents yet.</td></tr>`;
    return;
  }
...
function renderTaskBoard(channels, tasks) {
  if (!channels.length) {
    els.taskBoard.innerHTML = `<div class="muted">No channels yet.</div>`;
    return;
  }

  const tasksByChannel = groupTasksByChannel(tasks);

  els.taskBoard.innerHTML = channels
    .map((ch) => {
      const list = tasksByChannel.get(ch.id) || [];
...
async function refresh() {
  ...
  renderTaskBoard(channels, tasks);
  renderEvents(events);
}
...
async function main() {
  ...
  els.createTaskBtn.addEventListener('click', async () => {
    ...
    await api('/v1/tasks', {
      method: 'POST',
      body: JSON.stringify({ channel_id, title, description, status: 'queued', priority: 0 }),
    });
    els.taskTitle.value = '';
    els.taskDesc.value = '';
    await refresh();
